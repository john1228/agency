class ReportController < ApplicationController
  ...
  def order
    ...
    @orders = Order.joins(:order_item).where(updated_at: date.at_beginning_of_month..date.at_end_of_month).where(filter).order(updated_at: :desc).paginate(page: params[:page]||1, per_page: 10)
    render layout: false
  end
